...
parser.add_argument('--result_dir', type=str, default='/extra/yadongl10/git_project/nlpresult/20201015')
parser.add_argument('--gpu', type=str, default='0')
parser.add_argument('--alter_prior', type=str, default='imom')
parser.add_argument('--phi', type=int, default=1)
parser.add_argument('--rho', type=float, default=0.25)
parser.add_argument('--p', type=int, default=500,  help='number of covariates')
...
def train(Y, X, truetheta, epoch, alter_prior, tau, rep, lr, scale, temparature, init, p, phi, rho, result_dir):
    ...
    with open(result_dir + '/{}_theta_posterior_p{}_phi{}_rho{}_repeat{}.pkl'.format(alter_prior, p, phi, rho, rep), 'wb') as f:
        pkl.dump(theta, f)
    #
    with open(result_dir + '/{}_sse_zero_list_p{}_phi{}_rho{}_repeat{}.pkl'.format(alter_prior, p, phi, rho, rep), 'wb') as f:
        pkl.dump(sse_zero_list, f)
    with open(result_dir + '/{}_sse_nonzero_list_p{}_phi{}_rho{}_repeat{}.pkl'.format(alter_prior, p, phi, rho, rep), 'wb') as f:
...
def main(args):
    ...
    p, phi, alter_prior, rho = args.p, args.phi, args.alter_prior, args.rho
    ...
    lr = args.lr  # sgd 0.0001  adam 0.005
    scale = 'learned'  # 1.1
    temparature = 9/10
    ...
    for i in range(rep):
        ...
        linear, best_theta, sse_nonzero_best, sse_zero_best, sse_theta = train(Y, X, truetheta, epoch=epochs,
                                                                               alter_prior=alter_prior,
                                                                               tau=tau, rep=i, lr=lr, p=p,
                                                                               scale=scale, temparature=temparature,
                                                                               init=init, phi=phi, rho=rho,
                                                                               result_dir=result_dir)

        sse_theta_ls.append([sse_theta, sse_nonzero_best, sse_zero_best])
        test(Y, X, linear)

        with open(result_dir + '/p{}_phi{}_rho{}_repeat{}.pkl'.format(p, phi, rho, i), 'wb') as f:
            pkl.dump(best_theta, f)

    with open(result_dir + '/p{}_phi{}_rho{}_sse_theta_ls_tau{}.pkl'.format(p, phi, rho, tau), 'wb') as f:
